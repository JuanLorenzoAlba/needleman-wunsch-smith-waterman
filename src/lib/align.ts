...
export function needlemanWunsch(a: string, b: string, p: ScoreParams): AlignResult {
  const m = a.length;
  const n = b.length;
  const matrix: Cell[][] = Array.from({ length: m + 1 }, () =>
    Array.from({ length: n + 1 }, () => ({ score: 0, sources: [], diagVal: null, upVal: null, leftVal: null }))
  );

  for (let i = 0; i <= m; i++) matrix[i][0] = { score: i * p.gap, sources: i === 0 ? [] : ["up"], diagVal: null, upVal: null, leftVal: null };
  for (let j = 0; j <= n; j++) matrix[0][j] = { score: j * p.gap, sources: j === 0 ? [] : ["left"], diagVal: null, upVal: null, leftVal: null };
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      ...
      matrix[i][j] = { score: best, sources, diagVal: diag, upVal: up, leftVal: left };
    }
  }
  ...
  return { algorithm: "nw", a, b, params: p, matrix, score: matrix[m][n].score, end, start: { i: 0, j: 0 }, path, alignedA, alignedB };
}

export function smithWaterman(a: string, b: string, p: ScoreParams): AlignResult {
  const m = a.length;
  const n = b.length;
  const matrix: Cell[][] = Array.from({ length: m + 1 }, () =>
    Array.from({ length: n + 1 }, () => ({ score: 0, sources: [], diagVal: null, upVal: null, leftVal: null }))
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      ...
      matrix[i][j] = { score: rawBest, sources, diagVal: diag, upVal: up, leftVal: left };
      if (rawBest > best) {
        best = rawBest;
        end = { i, j };
      }
  ...
  return { algorithm: "sw", a, b, params: p, matrix, score: best, end, start, path, alignedA, alignedB };
}
...
function traceback(matrix: Cell[][], from: { i: number; j: number }): PathStep[] {
  const backward: { i: number; j: number }[] = [];
  let { i, j } = from;
  backward.push({ i, j });
  while (matrix[i][j].sources.length > 0) {
    const move = matrix[i][j].sources[0]; // canonical: diag > up > left
    if (move === "diag") { i -= 1; j -= 1; }
    else if (move === "up") { i -= 1; }
    else { j -= 1; }
    backward.push({ i, j });
  }
  const forward = backward.reverse();
  return forward.map((cell, idx) => ({ ...cell, move: idx === 0 ? null : moveBetween(forward[idx - 1], cell) }));
}

function moveBetween(prev: { i: number; j: number }, cur: { i: number; j: number }): Move {
...
function buildAlignment(a: string, b: string, path: PathStep[]) {
  ...
  for (let k = 1; k < path.length; k++) {
    const { move } = path[k];
    const prev = path[k - 1];
    if (move === "diag") { alignedA += a[prev.i]; alignedB += b[prev.j]; }
    else if (move === "up") { alignedA += a[prev.i]; alignedB += "-"; }
    else if (move === "left") { alignedA += "-"; alignedB += b[prev.j]; }
  }
